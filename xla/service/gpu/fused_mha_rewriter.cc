...
Status SetName(HloModule* module, HloInstruction* fmha) {
  ...
  if (fmha->custom_call_target() == kCudnnfMHAScaleSoftmaxCallTarget) {
    module->SetAndUniquifyInstrName(fmha, "fmha-bmm-scale-softmax-bmm");
    return OkStatus();
  }
...
bool IsRankSupported(const HloInstruction* bmm) {
  return bmm->operand(0)->shape().dimensions().size() ==
         bmm->operand(1)->shape().dimensions().size();
}

bool IsBatchDimSizeSupported(const DotDimensionNumbers& dot_dims) {
  return dot_dims.lhs_batch_dimensions().size() ==
...
bool MatchBmm1UnfusedBiasSoftmaxBmm2(HloInstruction* softmax_input,
  ...
  auto unfused_scaled_bmm_subpattern = m::MultiplyAnyOrder(
      ...
      m::Broadcast(
          OptionalConvert(m::Constant(scale).WithPredicate(IsScalar))));

  auto no_select_pattern = m::AnyOf<HloInstruction>(
      // BMM + softmax
      m::Op(bmm_1).WithPredicate(IsBatchedMatmul).WithOneUse(),
  ...
  if (Match(softmax_input, no_select_pattern)) {
    custom_call_name = has_dropout
                           ? kCudnnfMHAScaleBiasSoftmaxDropoutCallTarget
                           : ((*bias) ? kCudnnfMHAScaleBiasSoftmaxCallTarget
                                      : kCudnnfMHAScaleSoftmaxCallTarget);
    if (has_dropout) {
      dropout_rate = GetDropoutRateFromHlo(dropout);
    }
    return true;
...
StatusOr<bool> FusedMHARewriter::Run(
  ...
  for (HloComputation* comp :
    ...
    for (HloInstruction* instr : comp->MakeInstructionPostOrder()) {
      ...
      if (bmm_1) VLOG(1) << "bmm_1: " << bmm_1->ToString();
      if (bmm_2) VLOG(1) << "bmm_2: " << bmm_2->ToString();
      if (bias) VLOG(1) << "bias: " << bias->ToString();
      if (mask) VLOG(1) << "mask: " << mask->ToString();
      if (scale) VLOG(1) << "scale: " << scale->ToString();
