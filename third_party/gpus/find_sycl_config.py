...
The path to inspect is specified through an environment variable
If no valid configuration is found, the script prints to stderr and
returns an error code.

The script takes a list of base directories specified by the SYCL_TOOLKIT_PATH
environment variable. The script looks for headers and library files in a
hard-coded set of subdirectories from these base paths. If SYCL_TOOLKIT_PATH
is not specified, a OS specific default is used:

  Linux:   /opt/intel/oneapi/compiler/latest

Some libraries also use alternative base directories from other environment
variables if they are specified. List of library-specific environment variables:
