...
class GpuExecutor : public internal::StreamExecutorInterface {
 ...
 public:
  ...
  explicit GpuExecutor(const PluginConfig& plugin_config)
      : device_(0), device_ordinal_(0), plugin_config_(plugin_config) {}
 ...
 private:
  ...
  // (supported on CUDA only)
  // tsl::Status LoadModuleFromCuBin(const char* cubin, GpuModuleHandle*
  // module)
  //     TF_EXCLUSIVE_LOCKS_REQUIRED(in_memory_modules_mu_);

  tsl::Status LoadModuleFromSpir(const char* spirv, const size_t size,
  ...
  // (supported on ROCm only)
  // tsl::Status LoadModuleFromHsaco(const char* hsaco, GpuModuleHandle*
  // module)
  //     TF_EXCLUSIVE_LOCKS_REQUIRED(in_memory_modules_mu_);
  ...
  std::unordered_map<const KernelBase*, const char*> kernel_to_gpu_binary_
      ABSL_GUARDED_BY(in_memory_modules_mu_);
  // GPU binary (PTX or CUBIN or HSACO) -> {CUDA module, reference count}.
  std::unordered_map<const void*, std::pair<ze_module_handle_t, uint64_t>>
