...
class BFCAllocator : public Allocator {
 public:
  ...
  std::string Name() override { return "itex_device_bfc"; }
 ...
 private:
  ...
  BinNum BinNumForSize(size_t bytes) {
    tsl::uint64 v = std::max<size_t>(bytes, 256) >> kMinAllocationBits;
    int b = std::min(kNumBins - 1, Log2FloorNonZero(v));
    return b;
  }

  // Returns floor(log2(n)).
  inline int Log2FloorNonZero(tsl::uint64 n) {
#if defined(__GNUC__)
  ...
  inline int Log2FloorNonZeroSlow(tsl::uint64 n) {
    int r = 0;
    while (n > 0) {
      r++;
      n >>= 1;
